...
diff --git a/tests/conftest.py b/tests/conftest.py
index 78973890de4d9d1be9d5bcf51c5e5e0ee5421bea..ff169bb08b576abc9fb1b569420c09369a003d0a 100644
--- a/tests/conftest.py
+++ b/tests/conftest.py
@@ -1,128 +1,252 @@
 # tests/conftest.py
...
diff --git a/rag/remote_embedder.py b/rag/remote_embedder.py
index 26f0648dce69e58729f69b07ec81c6d015aa551b..33f3832a6ce1ec418859de8813f49ce6acc1cc0a 100644
--- a/rag/remote_embedder.py
+++ b/rag/remote_embedder.py
@@ -1,76 +1,81 @@
 """
 ...
 from .exceptions import EmbeddingException, VMConnectionError, VMTimeoutError
 from .event_loop_manager import run_async_safe, get_shared_http_session
+from .transport_client import AiohttpTransportClient, TransportClientProtocol
 from .retry_policy import RetryPolicy, RetryConfig
 from .circuit_breaker import CircuitBreaker, CircuitBreakerConfig, CircuitBreakerOpenException
 from .vm_diagnostics import diagnose_vm_connection
...
+    ):
         ...
         self.embedding_dim = getattr(embedding_config, "embedding_dim", int(os.getenv("EMB_DIM", "1024")))
@@ -82,50 +87,55 @@ class RemoteVMEmbedder:
         self.retry_delay = float(os.getenv("RAG_RETRY_DELAY", str(self.remote_config.retry_delay)))
...
+        # Транспортный слой можно подменять в тестах
+        self.transport_client: TransportClientProtocol = (
+            transport_client or AiohttpTransportClient()
+        )
+
         # Статистика
         self.stats = {
...
@@ -287,74 +297,59 @@ class RemoteVMEmbedder:
                 retry_attempt=max_attempts  # Локальный контекст: достигли лимита
             )
...
diff --git a/rag/transport_client.py b/rag/transport_client.py
new file mode 100644
index 0000000000000000000000000000000000000000..eae5c4c8e985d72a3bcd8f174ba50de3d494c7cb
--- /dev/null
+++ b/rag/transport_client.py
@@ -0,0 +1,40 @@
+"""Транспортный слой для RemoteVMEmbedder."""
+from __future__ import annotations
+
...
+
+class TransportClientProtocol(Protocol):
+    """Протокол транспортного клиента для RemoteVMEmbedder."""
+
+    async def post_json(self, url: str, payload: Dict[str, Any]) -> Dict[str, Any]:
+        """Выполняет HTTP POST запрос и возвращает JSON ответ."""
+        ...
+
+
+class AiohttpTransportClient:
+    """Реализация транспортного клиента на базе aiohttp."""
+
+    async def post_json(self, url: str, payload: Dict[str, Any]) -> Dict[str, Any]:
+        session = await get_shared_http_session()
+
...
+            )
diff --git a/tests/conftest.py b/tests/conftest.py
index 78973890de4d9d1be9d5bcf51c5e5e0ee5421bea..731a90de8c6c7dde51370fe0d1c218ef47726b69 100644
--- a/tests/conftest.py
+++ b/tests/conftest.py
@@ -1,33 +1,38 @@
 # tests/conftest.py
...
+
 ...
 def ensure_utf8_subprocess(monkeypatch):
     """Гарантирует корректное декодирование stdout/stderr в subprocess.run."""
@@ -85,56 +90,100 @@ def pytest_configure(config):
                 search_embedder_patcher,
                 query_engine_embedder_patcher,
                 remote_embedder_patcher,
             ])
...
+    """Фабрика для создания RemoteVMEmbedder с возможностью подмены транспорта."""
+
+    def _create_embedder(
+        embedding_config: Optional[EmbeddingConfig] = None,
+        remote_service_config: Optional[RemoteServiceConfig] = None,
+        override_mock: bool = False,
+        transport_client: Optional[Any] = None,
+    ):
...
+        )
+
+        remote_cfg = remote_service_config or RemoteServiceConfig(
+            host="localhost",
+            port=8000,
+            timeout_seconds=5,
+            max_retries=3,
...
+                remote_service_config=remote_cfg,
+            )
+
+        module = importlib.import_module("rag.remote_embedder")
+        module = importlib.reload(module)
+        remote_cls = module.RemoteVMEmbedder
+
+        return remote_cls(
+            embedding_config=embedding_cfg,
+            remote_service_config=remote_cfg,
...
diff --git a/tests/mocks/mock_transport_client.py b/tests/mocks/mock_transport_client.py
new file mode 100644
index 0000000000000000000000000000000000000000..4924aaaaca6093bc216f9f914756d6593a01b604
--- /dev/null
+++ b/tests/mocks/mock_transport_client.py
@@ -0,0 +1,46 @@
+"""Модуль с тестовым транспортным клиентом для RemoteVMEmbedder.
+
+Класс предназначен для подмены HTTP транспорта в тестах без
+патчинга приватных методов. Он отслеживает количество вызовов и
+позволяет управлять сценариями ошибок.
+"""
+from __future__ import annotations
+
+import asyncio
...
+    """Простая реализация транспортного клиента для тестов."""
+
+    def __init__(self) -> None:
+        self.call_count: int = 0
+        self.calls_history: List[Dict[str, Any]] = []
+        self.should_fail: bool = False
+        self.failures_before_success: Optional[int] = 0
+        self._failures_done: int = 0
+        self.exception_factory: Callable[[], Exception] = (
+            lambda: asyncio.TimeoutError("Mock transport timeout")
+        )
+        self.response_payload: Dict[str, Any] = {"embeddings": [[0.0, 0.0]]}
+        self.delay_seconds: float = 0.0
+
+    async def post_json(self, url: str, payload: Dict[str, Any]) -> Dict[str, Any]:
+        """Имитирует HTTP вызов к сервису эмбеддингов."""
+        self.call_count += 1
+        self.calls_history.append({"url": url, "payload": payload})
+
...
+            if self._failures_done < (self.failures_before_success or 0):
+                self._failures_done += 1
+                raise self.exception_factory()
+            # После серии ошибок сбрасываем счётчик, чтобы последующие
+            # вызовы могли снова симулировать неудачи при необходимости.
+            self._failures_done = 0
+
+        return self.response_payload
...
-    """
+def test_timeout_formula_sync(embedder_factory):
+    """Формула timeout должна быть синхронизирована с расчётами в коде."""
     base_timeout = 10.0
     max_retries = 3
     retry_delay = 2.0
-    
...
+
+    embedder_factory(remote_service_config=remote_config)
+
+    num_backoff_intervals = max(0, max_retries - 1)
...
+async def test_circuit_breaker_open_no_retry(embedder_factory):
+    """CircuitBreakerOpenException не должен запускать retry."""
+    embedder = embedder_factory()
+
     for _ in range(10):
         try:
...
diff --git a/pytest.ini b/pytest.ini
index 83feda93ede2f74f80fab629dcb8b694e6f057b5..bed65c65528948cd416df8e28c18b56dc6bcdb60 100644
--- a/pytest.ini
+++ b/pytest.ini
@@ -1,20 +1,22 @@
 [pytest]
 ...
 markers =
     ...
     real: Тесты с реальными компонентами (требуют Qdrant)
     enable_socket: Enable socket functionality for tests
+    real_embedder: Тест использует реальный RemoteVMEmbedder
+    mock_embedder: Тест использует MockRemoteEmbedder
 filterwarnings =
     ignore::pytest.PytestUnhandledThreadExceptionWarning
 asyncio_mode = auto
 asyncio_default_fixture_loop_scope = function
diff --git a/tests/conftest.py b/tests/conftest.py
index 78973890de4d9d1be9d5bcf51c5e5e0ee5421bea..e19f087e9b66d19160c8c44f32afbd93726a21b3 100644
--- a/tests/conftest.py
+++ b/tests/conftest.py
@@ -1,55 +1,74 @@
 # tests/conftest.py
...
+
 ...
 @pytest.fixture(autouse=True)
-def force_offline_env(monkeypatch):
+def force_offline_env(monkeypatch: pytest.MonkeyPatch, request: pytest.FixtureRequest):
     """Гарантирует offline-профиль по умолчанию для тестов"""
+
+    needs_real_embedder = "real_embedder" in request.keywords
+
     monkeypatch.setenv("PYTHONIOENCODING", "utf-8")
     monkeypatch.setenv("PYTHONUTF8", "1")
     monkeypatch.setenv("OFFLINE_MODE", "1")
-    monkeypatch.setenv("USE_MOCK_EMBEDDER", "1")
+    monkeypatch.setenv("USE_MOCK_EMBEDDER", "0" if needs_real_embedder else "1")
     monkeypatch.setenv("USE_MOCK_VECTOR_STORE", "1")
     monkeypatch.setenv("EMBEDDING_PROVIDER", os.getenv("EMBEDDING_PROVIDER", "mock"))
     monkeypatch.setenv("VECTOR_STORE_PROVIDER", os.getenv("VECTOR_STORE_PROVIDER", "mock"))
     monkeypatch.setenv("HF_HUB_OFFLINE", "1")
...
+
+    remote_patcher = None
+    if needs_real_embedder:
+        for patcher in getattr(request.config, "_mock_patchers", []):
+            if getattr(patcher, "attribute", "") == "RemoteVMEmbedder":
+                remote_patcher = patcher
+                try:
+                    remote_patcher.stop()
+                except RuntimeError:
+                    pass
...
+    if needs_real_embedder and remote_patcher is not None:
...
+
 ...
 def pytest_configure(config):
     ...
     # Проверяем, нужно ли использовать mock эмбеддеры
@@ -85,56 +104,103 @@ def pytest_configure(config):
                 search_embedder_patcher,
                 query_engine_embedder_patcher,
                 remote_embedder_patcher,
             ])
...
+@pytest.fixture
+def embedder_factory(request: pytest.FixtureRequest) -> Callable[..., Any]:
+    """Создает real или mock RemoteVMEmbedder в зависимости от маркеров теста."""
+
+    def _create_embedder(
+        *,
+        override_mock: Optional[bool] = None,
...
+        provider: Optional[str] = None,
+    ) -> Any:
+        """Возвращает готовый эмбеддер с учетом маркеров и переменных окружения."""
+
+        marker_real = "real_embedder" in request.keywords
+        marker_mock = "mock_embedder" in request.keywords
+
+        if marker_real and marker_mock and override_mock is None:
+            raise RuntimeError("Нельзя одновременно использовать маркеры real_embedder и mock_embedder.")
+
+        if override_mock is not None:
+            use_mock = override_mock
+        elif marker_real:
+            use_mock = False
+        elif marker_mock:
+            use_mock = True
+        else:
+            env_value = os.getenv("USE_MOCK_EMBEDDER", "0").lower()
+            use_mock = env_value in {"1", "true", "yes"}
+
+        if use_mock:
+            from tests.mocks.mock_remote_embedder import MockRemoteEmbedder
+
...
diff --git a/pytest.ini b/pytest.ini
index 83feda93ede2f74f80fab629dcb8b694e6f057b5..fa3fb9e9841dbea0b611d936f7cbe15a18bda65d 100644
--- a/pytest.ini
+++ b/pytest.ini
@@ -1,20 +1,21 @@
 [pytest]
 ...
 markers =
     ...
     mock: Тесты с mock объектами
     real: Тесты с реальными компонентами (требуют Qdrant)
+    vm: Тесты, требующие доступности удалённой VM
     enable_socket: Enable socket functionality for tests
 filterwarnings =
     ignore::pytest.PytestUnhandledThreadExceptionWarning
 asyncio_mode = auto
 asyncio_default_fixture_loop_scope = function
diff --git a/tests/conftest.py b/tests/conftest.py
index 78973890de4d9d1be9d5bcf51c5e5e0ee5421bea..f603a0f7a91ba7d64d550ccb4ed798462d3949f3 100644
--- a/tests/conftest.py
+++ b/tests/conftest.py
@@ -1,73 +1,123 @@
 # tests/conftest.py
...
+        action="store",
+        default=None,
+        help="Переопределяет хост VM сервиса для @pytest.mark.vm тестов"
+    )
+    parser.addoption(
+        "--vm-port",
+        action="store",
+        type=int,
+        default=None,
+        help="Переопределяет порт VM сервиса для @pytest.mark.vm тестов"
+    )
+
+
+def check_vm_availability(host: str, port: int, timeout: float = 0.5) -> bool:
+    """Проверяет доступность удалённого VM сервиса через TCP подключение."""
+
+    import socket
+
+    try:
...
+
+
+def _resolve_vm_endpoint(config: "pytest.Config") -> tuple[str, int]:
+    """Возвращает параметры подключения к VM с учётом CLI и переменных окружения."""
+
+    vm_host = config.getoption("--vm-host") or os.getenv("RAG_SERVICE_HOST", "10.61.11.54")
...
+
+def pytest_collection_modifyitems(config, items):
+    """Добавляет пропуски для VM тестов, если удалённый сервис недоступен."""
+
+    vm_host, vm_port = _resolve_vm_endpoint(config)
+
+    for item in items:
+        if "vm" in item.keywords and not check_vm_availability(vm_host, vm_port, timeout=0.5):
+            item.add_marker(
+                pytest.mark.skip(
+                    reason=(
...
index a2fdba49ded3216afb0bcc6918cf0878f93a08d1..ec5a7434cad83a9b793a83a60a145df68685babe 100644
--- a/README.md
+++ b/README.md
@@ -338,50 +338,53 @@ python run_web.py  # запустить UI для тестирования
 ---
...
diff --git a/pytest.ini b/pytest.ini
index 83feda93ede2f74f80fab629dcb8b694e6f057b5..f92dfc3358bcaea3c3da5effe1d93b8269246a4f 100644
--- a/pytest.ini
+++ b/pytest.ini
@@ -1,20 +1,22 @@
 [pytest]
 ...
 markers =
     ...
     mock: Тесты с mock объектами
     real: Тесты с реальными компонентами (требуют Qdrant)
+    real_embedder: Тесты, требующие реального RemoteVMEmbedder через фабрику
+    mock_embedder: Тесты, использующие MockRemoteEmbedder через фабрику
     enable_socket: Enable socket functionality for tests
 filterwarnings =
     ignore::pytest.PytestUnhandledThreadExceptionWarning
 asyncio_mode = auto
 asyncio_default_fixture_loop_scope = function
diff --git a/tests/conftest.py b/tests/conftest.py
index 78973890de4d9d1be9d5bcf51c5e5e0ee5421bea..a730a6da067023a1ca4fb7b69346371c0c5bda3b 100644
--- a/tests/conftest.py
+++ b/tests/conftest.py
@@ -1,31 +1,34 @@
 # tests/conftest.py
...
+from typing import Any, Callable, Optional
 ...
 @pytest.fixture(autouse=True)
@@ -114,27 +117,95 @@ def reset_embedder_environment():
...
+
+
+def _load_real_remote_embedder_class():
+    """Загружает оригинальный класс RemoteVMEmbedder, обходя глобальные патчи."""
+    module_path = Path(__file__).resolve().parent.parent / "rag" / "remote_embedder.py"
+    spec = importlib.util.spec_from_file_location(
+        "rag._remote_vm_embedder_real",
+        module_path,
+    )
...
+    return module.RemoteVMEmbedder
+
+
+@pytest.fixture
+def embedder_factory(request) -> Callable[..., Any]:
+    """Фабрика для создания инстансов RemoteVMEmbedder или MockRemoteEmbedder."""
+
+    def _resolve_should_mock(override_mock: Optional[bool]) -> bool:
+        if override_mock is not None:
+            return override_mock
+
+        marker_for_real = request.node.get_closest_marker("real_embedder")
+        marker_for_mock = request.node.get_closest_marker("mock_embedder")
+
+        if marker_for_real and marker_for_mock:
+            raise RuntimeError(
+                "Нельзя одновременно использовать маркеры real_embedder и mock_embedder"
+            )
+
+        if marker_for_real:
+            return False
...
+            return True
+
+        return os.getenv("USE_MOCK_EMBEDDER", "1").lower() in {"1", "true", "yes"}
+
+    def _factory(
+        *,
+        override_mock: Optional[bool] = None,
...
+        **kwargs: Any,
+    ) -> Any:
+        should_mock = _resolve_should_mock(override_mock)
+
+        if should_mock:
+            from tests.mocks.mock_remote_embedder import MockRemoteEmbedder
+
...
+            )
+
+        remote_class = _load_real_remote_embedder_class()
+        return remote_class(
+            embedding_config=embedding_config,
+            parallelism_config=parallelism_config,
+            remote_service_config=remote_service_config,
...
+    return _factory
diff --git a/tests/mocks/mock_remote_embedder.py b/tests/mocks/mock_remote_embedder.py
index 7d83e238be46fb70462e0fe91bcefad2f15d3a21..068984ae4b1e4919fd6deb06d17d35f14615e770 100644
--- a/tests/mocks/mock_remote_embedder.py
+++ b/tests/mocks/mock_remote_embedder.py
@@ -1,86 +1,182 @@
 import asyncio
...
+from typing import Any, Dict, List, Optional
 ...
 class MockRemoteEmbedder:
     ...
     def __init__(self, embedding_config=None, parallelism_config=None, remote_service_config=None):
         ...
         self.provider_name = getattr(embedding_config, "provider", "mock-remote") if embedding_config else "mock-remote"
-        self.stats = {
+        self._service_url = "mock://remote-embedder"
+        self._is_warmed_up = True
+        self._stats: Dict[str, Any] = {
             "total_requests": 0,
             "total_texts": 0,
...
+        self._retry_stats: Dict[str, float] = {
+            "total_executions": 0,
+            "total_retries": 0,
+            "successful_executions": 0,
+            "failed_executions": 0,
...
+        self._retry_stats["total_executions"] += 1
+        self._retry_stats["successful_executions"] += 1
+
+        vectors = [self._create_vector(text) for text in texts]
...
+
+    def get_stats(self) -> Dict[str, Any]:
+        """Возвращает статистику, совместимую с RemoteVMEmbedder."""
+        total_requests = self._stats["total_requests"]
+        avg_response_time = (
+            self._stats["total_time"] / total_requests if total_requests else 0.0
+        )
+
+        retry_stats = self._retry_stats.copy()
+        total_exec = retry_stats["total_executions"]
+        if total_exec:
+            retry_stats["success_rate"] = (
+                retry_stats["successful_executions"] / total_exec * 100
+            )
+            retry_stats["avg_retries_per_execution"] = (
+                retry_stats["total_retries"] / total_exec
+            )
+        else:
+            retry_stats["success_rate"] = 0.0
+            retry_stats["avg_retries_per_execution"] = 0.0
+
+        cb_state = self._cb_state.copy()
+        if "time_until_retry" not in cb_state:
+            cb_state["time_until_retry"] = 0.0
+
+        return {
...
+
+    def check_health(self) -> Dict[str, Any]:
+        """Возвращает простую диагностику состояния mock эмбеддера."""
+        return {
+            "status": "connected",
+            "details": {
+                "provider": self.provider_name,
...
+            },
+        }
diff --git a/tests/test_remote_clients.py b/tests/test_remote_clients.py
index 76141d4abe28691a2ab1682cf2fc5e8991da7446..00c6585e2d6715cbb52d72fd5118102dc9762fe5 100644
--- a/tests/test_remote_clients.py
+++ b/tests/test_remote_clients.py
@@ -1,49 +1,50 @@
 import numpy as np
 import pytest
 from rag.exceptions import EmbeddingException
...
-    embedder = RemoteVMEmbedder()
+def test_remote_embedder_sync_wrapper(monkeypatch, texts, embedder_factory):
...
-    embedder = RemoteVMEmbedder()
+@pytest.mark.real_embedder
+def test_remote_embedder_fallback(monkeypatch, embedder_factory):
+    embedder = embedder_factory(override_mock=False)
...
diff --git a/tests/test_remote_embedder_fixes.py b/tests/test_remote_embedder_fixes.py
index 1b9ae5df7ebaa00b5f1d1f4dd8a65ddf0ff69097..cc9709496ec33299c6ea4c781a79ff9b7c66f356 100644
--- a/tests/test_remote_embedder_fixes.py
+++ b/tests/test_remote_embedder_fixes.py
@@ -1,208 +1,278 @@
 """
...
+def assert_stats_contract(stats):
+    """Проверяет, что структура статистики совместима с контрактом."""
+    required_keys = {
+        "total_requests",
+        "total_texts",
+        "total_time",
...
+    }
+    assert retry_required.issubset(retry_stats.keys())
+
+    cb_state = stats["circuit_breaker_state"]
...
+    }
+    assert cb_required.issubset(cb_state.keys())
+
+
 @pytest.fixture
-def embedder():
-    """Создаёт RemoteVMEmbedder с тестовой конфигурацией"""
+def embedder(request, embedder_factory):
+    """Создаёт embedder через фабрику с тестовой конфигурацией."""
     embedding_config = EmbeddingConfig(
         model_name="test-model",
         embedding_dim=1024
     )
...
+        override = False
+    elif request.node.get_closest_marker("mock_embedder"):
+        override = True
+
+    instance = embedder_factory(
+        override_mock=override,
         embedding_config=embedding_config,
         remote_service_config=remote_config
     )
 
+    reset = getattr(instance, "reset_stats", None)
+    if callable(reset):
+        reset()
+
+    return instance
...
-    test_embedder = RemoteVMEmbedder(remote_service_config=remote_config)
+    test_embedder = embedder_factory(
+        override_mock=False,
+        remote_service_config=remote_config
+    )
...
+@pytest.mark.mock_embedder
+def test_mock_embedder_stats_contract(embedder_factory):
