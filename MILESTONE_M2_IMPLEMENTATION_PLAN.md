...
- [ ] Проверить совместимость обновленных версий

### 🏗️ Фаза 2: Новые компоненты Sparse Encoding [День 3-4]

#### 2.1 Создание rag/sparse_encoder.py
- [ ] Создать базовый класс `BaseSparseEncoder`
- [ ] Реализовать `BM25Encoder` класс:
  - [ ] `__init__()` с конфигурацией BM25 параметров
  - [ ] `fit()` для построения vocabulary из корпуса
  - [ ] `encode()` для генерации sparse vectors
  - [ ] `batch_encode()` для батчевой обработки
- [ ] Реализовать `CodeTokenizer` для специальной токенизации:
  - [ ] Обработка camelCase: `getUserName` → `["get", "user", "name"]`
  - [ ] Обработка snake_case: `get_user_name` → `["get", "user", "name"]`
  - [ ] Обработка специальных символов: `__init__` → `["init"]`
  - [ ] Обработка точечной нотации: `self.config.value` → `["self", "config", "value"]`
- [ ] Добавить поддержку сохранения/загрузки vocabulary
- [ ] Реализовать метод `get_stats()` для мониторинга

#### 2.2 Создание тестов для sparse_encoder
- [ ] **tests/rag/test_sparse_encoder.py** - создать файл
- [ ] Тест инициализации BM25Encoder
- [ ] Тест токенизации различных стилей кода:
  - [ ] camelCase тесты
  - [ ] snake_case тесты  
  - [ ] Специальные символы тесты
  - [ ] Смешанные стили тесты
- [ ] Тест генерации sparse vectors:
  - [ ] Проверка размерности
  - [ ] Проверка sparsity (>95% нулей)
  - [ ] Проверка воспроизводимости
- [ ] Тест сохранения/загрузки vocabulary
- [ ] Performance тест: время кодирования 1000 документов

#### 2.3 Опциональная SPLADE интеграция (если GPU доступен)
- [ ] Добавить `SPLADEEncoder` класс
- [ ] Интегрировать с transformers библиотекой
- [ ] Добавить fallback на BM25 при отсутствии GPU
- [ ] Тесты для SPLADE (пропускать если нет GPU)
...
**⚠️ НЕ ЗАБУДЬТЕ УДАЛИТЬ ЭТОТ ФАЙЛ ПОСЛЕ ЗАВЕРШЕНИЯ MILESTONE M2 ⚠️**
