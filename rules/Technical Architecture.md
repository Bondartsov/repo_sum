...
- **Единый стандарт размерности**: 768d векторы (унифицировано для всех компонентов)
...
- **Adaptive HNSW**: динамические параметры (m=24, ef_construct=128 для 768d)
...
#### Context-Aware Factory Pattern (RAGFactory)
**Принцип**: Автоматический выбор реализации (Local vs Remote) в зависимости от среды выполнения (Client vs VM).
**Реализация**: `rag/factory.py`
- **VM Context**: Создает `CPUEmbedder` и `QdrantVectorStore` (прямой доступ).
- **Client Context**: Создает `RemoteVMEmbedder` и `RemoteVMVectorStore` (HTTP клиенты).
- **Цель**: Единый код инициализации для тестов, CLI и сервера без дублирования условий.
...
#### Hybrid Search Protocol (v1/search_v2)
- **Protocol**: Векторный протокол обмена данными (dense_vector + sparse_vector).
- **Endpoint**: `/v1/search_v2` для точного контроля над поиском.
- **Legacy**: `/v1/search` поддерживает текстовый query для обратной совместимости.
- **Fusion**: RRF (Reciprocal Rank Fusion) выполняется на стороне VM.
...
#### RAG Система (RAG-as-a-Service):
1. **RAGFactory** (rag/factory.py) - точка входа для создания компонентов.
2. **RemoteVMEmbedder** (rag/remote_embedder.py) - HTTP клиент эмбеддингов (Client-side).
3. **RemoteVMVectorStore** (rag/remote_vector_store.py) - HTTP клиент поиска (Client-side).
4. **CPUEmbedder** (rag/embedder.py) - серверный генератор эмбеддингов (VM-side).
5. **QdrantVectorStore** (rag/vector_store.py) - серверный клиент БД (VM-side).
6. **VM Service** (vm_rag_service.py) - FastAPI сервер, оркестрирующий потоки.
...
- **Jina Embeddings v3** - революционная модель с dual task архитектурой (570M параметров, 768d)
