...
- ✅ **Офлайн-режим OpenAI**: CLI и тесты получают детерминированную заглушку без сетевых вызовов
...
---
### 🗂️ **Multi-Repository Support Feature** (Запланировано)
**Статус:** 📋 ПЛАНИРОВАНИЕ  
**Приоритет:** Medium-High  
**Оценка:** 2-3 дня разработки + тестирование  
**Планируемый срок:** После стабилизации M3 или параллельно с M4

**Описание:**  
Расширение системы для поддержки множественных репозиториев с изолированным хранением и поиском.

**Основные компоненты:**

#### **1. Collection Management (Backend)**
- Динамическое создание коллекций Qdrant по имени репозитория
- Нормализация имён (санитизация для Qdrant naming rules)
- API для получения списка доступных коллекций
- Метаданные коллекций (repo path, indexed date, vector count)

#### **2. Configuration Layer**
- Параметр `collection_name` извлекается из имени репозитория
- Mapping: repo_path → normalized_collection_name
- Сохранение истории индексаций (какие репозитории когда индексировались)

#### **3. Web UI Enhancements**
- Dropdown/select для выбора репозитория перед поиском
- Отображение метрик по каждому репозиторию (кол-во чанков, языки)
- Индикация активной коллекции
- Возможность переиндексации выбранного репозитория

#### **4. Search Service Modifications**
- Добавление параметра `collection_name` в search queries
- Фильтрация результатов строго по выбранной коллекции
- Cross-repository search (опционально, для будущего)

#### **5. Testing Strategy**
- Unit-тесты для collection name normalization
- Integration-тесты с несколькими репозиториями
- UI-тесты для repository selection workflow
- Load-тесты с 5+ репозиториями

**Технические риски:**
- Qdrant collection naming constraints (длина, символы)
- Memory overhead при множественных коллекциях на VM
- UI complexity для большого числа репозиториев (>10)
- Конфликты имён при схожих названиях папок

**Зависимости:**
- Текущие bugfix'ы должны быть протестированы и стабилизированы
- VM должна иметь достаточно ресурсов для множественных коллекций

**Этапы реализации:**
1. Анализ и проектирование архитектуры (0.5 дня)
2. Backend: Collection management API (1 день)
3. Config layer: Dynamic collection naming (0.5 дня)
4. UI: Repository selector + metadata display (1 день)
5. Integration + testing (0.5-1 день)
6. Documentation update (0.5 дня)

**Ожидаемые преимущества:**
- Поддержка работы с несколькими проектами одновременно
- Изоляция данных между репозиториями
- Улучшенная организация и навигация
- Масштабируемость для команд с множественными проектами
