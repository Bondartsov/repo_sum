...
где работает Qdrant с 768d векторами и гибридным поиском.
"""
...
class RemoteVMVectorStore:
    ...
    async def _async_search(
        ...
        Args:
            query_vector: Dense вектор запроса (768d для Jina v3)
    ...
    async def _async_health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            qdrant_status = result.get("qdrant_status", "unknown")
            health_info["components"]["vector_store"]["qdrant_status"] = qdrant_status
            ...
            # P0-2: Check for degraded state
            if qdrant_status not in ("ok", "ready", "green"):
                 health_info["status"] = "degraded"
                 health_info["diagnostic"] = {
                    "error_type": "qdrant_not_ready",
                    "qdrant_status": qdrant_status,
                    "recommendation": "Qdrant сервис работает, но не готов к запросам (status != ok). Проверьте логи VM.",
