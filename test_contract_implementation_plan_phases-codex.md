# Implementation Plan: Test Contract Refactoring

## Фаза 0: Контекст и цели
...
## Фаза 1: Контракты и типы

[Types]
Определение формального контракта embedder через Protocol с runtime проверкой.

## 1. EmbedderProtocol (rag/embedder_protocol.py - создать овый файл)
...
## Фаза 2: Инвентаризация изменений
...
### 4. ✅ Code review
- Минимум один другой разработчик просмотрел изменения
- Все комментарии адресованы
...
- EmbedderProtocol с версионированной схемой get_stats()
- TransportClientProtocol для инжекции HTTP клиента
- Маркеры pytest: real_embedder, mock_embedder, vm
- Windows event loop policy для стабильности тестов
- VM пре-чек с автоматическим skip
- freezegun для стабилизации timeout тестов
- Линтер правило против приватных обращений в tests/
- CI/CD матрица: unit-real, unit-mock, vm джобы

### Changed
- Убран глобальный патч RemoteVMEmbedder из conftest.py
- Дефолт USE_MOCK_EMBEDDER изменен с "1" на "0"
- Mock классы реализуют полный EmbedderProtocol контракт
- Тесты переписаны на проверку публичного поведения
- get_stats() возвращает версионированную вложенную структуру

### Removed
- autouse=True из force_offline_env fixture
- Зависимости от приватных методов (_make_request_with_retry)
- Патчинг приватных методов в тестах

### Fixed
- AttributeError на приватных атрибутах в тестах
- Утечки состояния между тестами
- Флаки в timeout тестах (через freezegun)
- Windows event loop проблемы

### Breaking Changes
- USE_MOCK_EMBEDDER="1" больше не дефолтное значение
- Тесты требующие real embedder должны использовать @pytest.mark.real_embedder
- get_stats() возвращает новую структуру (обратная совместимость сохранена)
```
...
**Время реализации:** 17-20 часов (7 фаз)

**Риски:** Средние (checkpoint на каждой критической фазе)

**ROI:** Высокий
- Устранение технического долга
- Повышение стабильности тестов
- Упрощение разработки новых тестов
- Улучшение onboarding новых разработчиков

**Следующий шаг:** Начать Фазу 1, Шаг 1.1

---

**Дата создания:** 02.10.2025, 01:58 AM
**Версия документа:** 1.0.0 (Complete)
**Статус:** Ready for Implementation
**Автор:** AI Assistant + User Feedback

*Этот документ является comprehensive implementation plan и включает все критические уточнения от пользователя. План готов к немедленной реализации.*
