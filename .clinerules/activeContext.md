...
- **Ключевые компоненты**:
  - `embedder.py` - CPU-оптимизированный эмбеддер с int8 precision
...
- **Архитектура**:
  - **CPU-first подход**: sentence-transformers 5.1.0 с precision='int8'
  - **Qdrant кластер**: m=24, ef_construct=128, ScalarQuantization, replication_factor=2
  - **Гибридный поиск**: dense + sparse (BM25/SPLADE) + RRF
