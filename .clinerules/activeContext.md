...
**Статус:** MILESTONE M2 ✅ ЗАВЕРШЁН - ГИБРИДНЫЙ ПОИСК BM25/SPLADE
**Версия:** 1.8.0 (M2)
**Ветка:** main
...
### ✅ ЗАВЕРШЁННАЯ ФАЗА: ФАЗА 1.1 - Критические баги CPUQueryEngine

**Статус**: ✅ **ПОЛНОСТЬЮ ЗАВЕРШЕНА** (4 сентября 2025)

**Выполненные исправления:**
- ✅ **Bug #1**: Исправлен `_check_vector_store()` - заменён `is_connected()` на `health_check()`
- ✅ **Bug #2**: Исправлен `_ensure_embeddings()` - заменён `embedding_dim` на `config.vector_store.vector_size`
- ✅ **Unit тесты**: Создан `tests/rag/test_query_engine_health.py` с 7 тестами
- ✅ **Валидация**: Команда `rag status` работает корректно, MMR fallback с правильной размерностью (384)

### 🔄 Текущая фаза: ФАЗА 1.2 - Остальные критические баги

**Прогресс Фазы 1.2:**
- ✅ CPUQueryEngine баги исправлены (Фаза 1.1)
- [x] Исправить SearchService `min_score=0.0` логику (выполнено 05.09.2025)
- [x] Синхронизировать пороги релевантности (0.5 везде) (выполнено 05.09.2025)
- [x] Унифицировать контракт статистики токенов (выполнено 05.09.2025)
- [x] Актуализировать README.md документацию (выполнено 05.09.2025)
...
### ✅ Исправленные критические баги M2:
1. ✅ **QueryEngine.health_check()** - ИСПРАВЛЕНО: заменён `is_connected()` на `health_check()` API
2. ✅ **QueryEngine._ensure_embeddings()** - ИСПРАВЛЕНО: заменён `embedding_dim` на `config.vector_store.vector_size`

### 🐛 Оставшиеся критические баги для исправления:
3. **SearchService** - некорректная обработка `min_score=0.0`
4. **Config** - конфликт score_threshold (0.7 vs 0.5)
5. **Token stats** - несоответствие контракта UI/OpenAIManager
6. **README.md** - устаревшая документация
...
### Фазы:
- ✅ **Фаза 1.1** (ЗАВЕРШЕНА): CPUQueryEngine баги - 1 день (4.09.2025)
- 🔄 **Фаза 1.2** (ТЕКУЩАЯ): Остальные критические баги - 1 день
...
**ЗАКЛЮЧЕНИЕ**: Milestone M2 завершён. Реализован гибридный поиск BM25/SPLADE с RRF fusion, все тесты проходят успешно. Все изменения влиты в ветку `main`.
