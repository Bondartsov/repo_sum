...
**Статус:** Production-Ready с RAG системой + выявленным техническим долгом  
**Версия:** 1.7.0 + Technical Debt Audit
...
## 🚨 Выявленные критические проблемы (04.09.2025)

### ❌ **КРИТИЧЕСКИЕ ТЕХНИЧЕСКИЕ ПРОБЛЕМЫ**:

#### 1. QueryEngine.health_check() - несуществующий метод API
...
```
**Корень проблемы**: `QdrantVectorStore` не имеет метода `is_connected()`
**Влияние**: 
- Некорректная диагностика RAG системы
- Ложные статусы здоровья в `rag status`
- Потенциальные сбои в production мониторинге

#### 2. Несоответствие контрактов статистики токенов
**Проблемные файлы**: 
- `openai_integration.py:205` - `get_token_usage_stats()`
- `main.py` и `web_ui.py` - потребители API
**Проблема**:
```python
# UI ожидает:
stats = {"used_today": 1500}

# OpenAIManager возвращает:
stats = {"total_requests": 10, "total_tokens": 1500, "average_tokens_per_request": 150}
```
**Влияние**:
- Неработающие метрики в CLI и Web UI
- Потенциальные ошибки KeyError при обращении к `stats["used_today"]`

#### 3. Конфликт дефолтных значений порога релевантности
**Проблемные файлы**:
- `config.py:134` - `QueryEngineConfig.score_threshold = 0.7`
- `rag/search_service.py:127` - `min_score or 0.5`
**Проблема**: Различные дефолтные значения в разных частях системы
**Влияние**:
- Непредсказуемые результаты поиска между окружениями
- Конфиг-дрифт между dev и production
- Сложности в воспроизведении результатов

#### 4. README.md устаревшая документация
**Проблемные секции**:
- Упоминания `openai.max_tokens_per_chunk`, `max_response_tokens`
- Неверная структура конфигурации RAG
- Несуществующие CLI команды (`rag clear`, `--use-mmr`)
**Влияние**:
- Ложные ожидания пользователей
- Ошибки при настройке системы
- Увеличенное время онбординга

### ⚠️ **ЧАСТИЧНЫЕ ПРОБЛЕМЫ**:

#### 5. SearchService логическая ошибка с min_score=0.0
**Файл**: `rag/search_service.py:127`
**Проблема**: `min_score or 0.5` теряет значение 0.0 (falsy)
**Решение**: `min_score if min_score is not None else 0.5`

#### 6. Отсутствие score_threshold в статистике поиска
**Файл**: `rag/search_service.py` метод `get_search_stats()`
**Проблема**: Ключевой параметр не возвращается
**Влияние**: Неполная наблюдаемость системы

### 📋 **ПЛАН УСТРАНЕНИЯ ПРОБЛЕМ**:
1. **Критические** (1-2 дня):
   - Исправить `QueryEngine.health_check()` → использовать `vector_store.health_check()`
   - Унифицировать контракт статистики токенов
   - Синхронизировать пороги релевантности на 0.5
   - Актуализировать README.md

2. **Частичные** (3-5 дней):
   - Исправить логику `min_score` в SearchService
   - Добавить `score_threshold` в статистику
   - Обновить версии зависимостей
   - Очистить устаревшие комментарии

---

**Техническая архитектура готова к дальнейшему развитию**: milestone M2-M4 имеют прочную основу для реализации гибридного поиска, enterprise развёртывания и производственного мониторинга. **Выявленный технический долг** задокументирован и готов к планомерному устранению.
