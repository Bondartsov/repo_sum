...
### Ключевые параметры:
- `max_response_tokens` - лимит токенов в ответе GPT (5000)
- `retry_attempts` - попытки при ошибках API (3)
- `chunk_strategy` - стратегия разбивки кода (logical/size/lines)
- `prompts.code_analysis_prompt_file` - путь к файлу с промптом
